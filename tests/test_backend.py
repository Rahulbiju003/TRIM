...
class TestComplete:
    ...
    async def test_returns_completion_result(self, mock_litellm):
        ...
        result = await backend.complete("system prompt", "user message")
        assert isinstance(result, CompletionResult)

    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_content_is_stripped(self, mock_litellm):
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_token_counts_extracted(self, mock_litellm):
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_model_propagated(self, mock_litellm):
        mock_litellm.return_value = _make_litellm_response()
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_passes_system_and_user_messages(self, mock_litellm):
        mock_litellm.return_value = _make_litellm_response()
        backend = LiteLLMBackend(model="m")
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_passes_timeout(self, mock_litellm, monkeypatch):
        mock_litellm.return_value = _make_litellm_response()
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_temperature_passed_when_set(self, mock_litellm, mock_info):
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_temperature_omitted_when_none(self, mock_litellm, monkeypatch):
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_raises_on_litellm_error(self, mock_litellm):
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_none_content_becomes_empty_string(self, mock_litellm):
    ...
    @patch("worker.backends.litellm_backend.litellm.completion")
    async def test_missing_usage_defaults_to_zero(self, mock_litellm):
